...
hook.Add("RenderScreenspaceEffects", "BeatrunNoclipBW", function()
	...
	local inp = color ~= 1
	local noclipping = ply:GetMoveType() == MOVETYPE_NOCLIP and not BuildMode and ply:GetMantle() == 0 and ply:GetClimbing() == 0 and not IsValid(ply:GetLadder()) and not ply:InVehicle() -- Backport
...
local function BeatrunHUD()
	...
	--local coursename = nil --Trying to prevent an error GMod keeps throwing at me
